DL_CONDITIONS_MENU = {
	...
	{ text="Addon is not Active", value=81, params=12, desc="Checks if an Addon is not active." },
}
...
DL_CheckCondition[72] = function(conditions)
	if getglobal(conditions.text)() ~= nil then
	if (not getglobal(conditions.text)()) then
		return true;
	end end
end
...
-- Addon is not Active
DL_CheckCondition[81] = function(conditions)
	local _, _, _, addon = GetAddOnInfo(conditions.text)
	if addon == nil or addon == 0 then addon = true; else addon = false end
	return addon
end