...
int BankParser(int argc, char **argv){
  ...
  //bank
  char num[100], sname[100], name[100], desc[1000], pic[100], type[100], id[10], subtype[100], owner[20];
      ...
      if (strstr(buf,"[\"")!=NULL && strstr(buf,"\"]")!=NULL)
      {
        ...
        while (strncmp(buf, "},", 2)!=0)
        {
          ...
          if (strncmp(buf, "},", 2)==0 && name[0]!='\0')
          {
            ...
            strcat(link,"cellspacing=0 border=0 valign=top align=left><tr><td><span style=\"font-size: 12px;");
            ...
            strcat(link, STDIMAGESFOLDER);
            strcat(link, "copper.gif\">");
            strcat(link,"</span></td></tr></table></div></td>");
